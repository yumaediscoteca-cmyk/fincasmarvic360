      ...
      <div className="bg-card/80 border border-border rounded-xl overflow-hidden">
        <div className="bg-muted/50 px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-black px-1.5 py-0.5 rounded border text-primary border-primary/40 bg-primary/5">A</span>
            <Building2 className="w-3.5 h-3.5 text-muted-foreground" />
            <span className="text-[10px] font-black uppercase tracking-widest text-foreground">Estado Finca / Parcela</span>
          </div>
          {esHoy && parteId && (
            <button
        ...
        <div className="fixed inset-0 z-modal flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto text-card-foreground shadow-lg">
            <div className="sticky top-0 bg-card border-b border-border px-5 py-3 flex items-center justify-between">
              <span className="text-sm font-black uppercase tracking-widest text-primary">A · Estado Finca / Parcela</span>
            ...
            <div className="p-5 space-y-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Finca *</label><select value={formA.finca} onChange={e => setFormA(p => ({ ...p, finca: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none"><option value="">— Seleccionar finca —</option>{FINCAS.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Parcela / Sector (opcional)</label><input type="text" value={formA.parcel_id} onChange={e => setFormA(p => ({ ...p, parcel_id: e.target.value }))} placeholder="Ej: S-12, Sector Norte..." className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Estado actual</label><select value={formA.estado} onChange={e => setFormA(p => ({ ...p, estado: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none"><option value="">— Sin especificar —</option>{ESTADOS_PARCELA.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Nº Operarios</label><input type="number" min="0" value={formA.num_operarios} onChange={e => setFormA(p => ({ ...p, num_operarios: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Nombres</label><input type="text" value={formA.nombres_operarios} onChange={e => setFormA(p => ({ ...p, nombres_operarios: e.target.value }))} placeholder="Juan, Pedro..." className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[1, 2].map(n => (
                  <div key={n}><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Foto {n} {n === 1 ? '(estado)' : '(opcional)'}</label><label className="flex items-center gap-2 px-3 py-2.5 bg-background border border-input rounded-lg cursor-pointer hover:border-primary/30 transition-colors"><Camera className="w-4 h-4 text-muted-foreground shrink-0" /><span className="text-[11px] text-muted-foreground truncate">{n === 1 ? (formA.foto1?.name ?? 'Capturar / Subir') : (formA.foto2?.name ?? 'Capturar / Subir')}</span><input type="file" accept="image/*" capture="environment" className="sr-only" onChange={e => { const f = e.target.files?.[0] ?? null; setFormA(p => n === 1 ? { ...p, foto1: f } : { ...p, foto2: f }) }} /></label></div>
                ))}
              </div>
