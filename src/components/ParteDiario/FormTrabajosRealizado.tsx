    ...
    <>
      <div className="bg-card/80 border border-border rounded-xl overflow-hidden">
        <div className="bg-muted/50 px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-black px-1.5 py-0.5 rounded border text-amber-600 dark:text-amber-400 border-amber-500/40 bg-amber-500/5">B</span>
            <Wrench className="w-3.5 h-3.5 text-muted-foreground" />
            <span className="text-[10px] font-black uppercase tracking-widest text-foreground">Trabajo en Curso</span>
          </div>
          {esHoy && parteId && (
            <button
              onClick={() => { setFormB(initB()); setEditIdB(null); setModalOpen(true); }}
              className="flex items-center gap-1 px-2.5 py-1 rounded border border-primary/30 bg-primary/5 hover:bg-primary/15 text-primary text-[10px] font-black uppercase tracking-widest transition-all"
            >
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-modal flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto text-card-foreground shadow-lg">
            <div className="sticky top-0 bg-card border-b border-border px-5 py-3 flex items-center justify-between">
              <span className="text-sm font-black uppercase tracking-widest text-amber-600 dark:text-amber-400">B · Trabajo en Curso</span>
              <button type="button" onClick={() => setModalOpen(false)} className="text-muted-foreground hover:text-foreground text-lg leading-none">×</button>
            </div>
            <div className="p-5 space-y-4">
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Tipo de trabajo *</label><input list="tipos-trabajo" value={formB.tipo_trabajo} onChange={e => setFormB(p => ({ ...p, tipo_trabajo: e.target.value }))} placeholder="Seleccionar o escribir..." className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /><datalist id="tipos-trabajo">{TIPOS_TRABAJO.map(t => <option key={t} value={t} />)}</datalist></div>
              <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Finca</label><select value={formB.finca} onChange={e => setFormB(p => ({ ...p, finca: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none"><option value="">— Sin especificar —</option>{FINCAS.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Ámbito</label>
                <div className="flex gap-2">
                  {[{ value: 'finca_completa', label: 'Finca completa' }, { value: 'parcelas_concretas', label: 'Parcelas concretas' }].map(opt => (
                    <button type="button" key={opt.value} onClick={() => setFormB(p => ({ ...p, ambito: opt.value }))} className={`flex-1 py-2 rounded-lg border text-[11px] font-bold transition-colors ${formB.ambito === opt.value ? 'border-primary/50 bg-primary/10 text-primary' : 'border-border text-muted-foreground hover:border-primary/30'}`}>{opt.label}</button>
                  ))}
                </div>
              </div>
              {formB.ambito === 'parcelas_concretas' && <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Parcelas (separadas por coma)</label><input type="text" value={formB.parcelas} onChange={e => setFormB(p => ({ ...p, parcelas: e.target.value }))} placeholder="S-01, S-02, S-03..." className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>}
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Hora inicio</label><input type="time" value={formB.hora_inicio} onChange={e => setFormB(p => ({ ...p, hora_inicio: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Hora fin</label><input type="time" value={formB.hora_fin} onChange={e => setFormB(p => ({ ...p, hora_fin: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Nº Operarios</label><input type="number" min="0" value={formB.num_operarios} onChange={e => setFormB(p => ({ ...p, num_operarios: e.target.value }))} className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
                <div><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Nombres</label><input type="text" value={formB.nombres_operarios} onChange={e => setFormB(p => ({ ...p, nombres_operarios: e.target.value }))} placeholder="Juan, Pedro..." className="w-full bg-background border border-input rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-ring/30 outline-none" /></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[1, 2].map(n => (
                  <div key={n}><label className="block text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1.5">Foto {n}</label><label className="flex items-center gap-2 px-3 py-2.5 bg-background border border-input rounded-lg cursor-pointer hover:border-primary/30 transition-colors"><Camera className="w-4 h-4 text-muted-foreground shrink-0" /><span className="text-[11px] text-muted-foreground truncate">{n === 1 ? (formB.foto1?.name ?? 'Capturar / Subir') : (formB.foto2?.name ?? 'Capturar / Subir')}</span><input type="file" accept="image/*" capture="environment" className="sr-only" onChange={e => { const f = e.target.files?.[0] ?? null; setFormB(p => n === 1 ? { ...p, foto1: f } : { ...p, foto2: f }) }} /></label></div>
                ))}
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setModalOpen(false)} className="flex-1 py-2.5 rounded-lg border border-border text-muted-foreground text-sm hover:bg-muted transition-colors">Cancelar</button>
